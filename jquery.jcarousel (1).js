...
(function ($) {
  ...
  jCarousel.base = function (pluginName) {
    return {
      ...
      carousel: function () {
        if (!this._carousel) {
          this._carousel = jCarousel.detectCarousel(
            this.options("carousel") || this._element
          );

          if (!this._carousel) {
            $.error(
              'Could not detect carousel for plugin "' + pluginName + '"'
            );
          }
        }
  ...
  $.jCarousel.plugin("jcarousel", {
    ...
    _reload: function () {
      ...
      if (this.vertical == null) {
        this.vertical =
          toFloat(this.list().height()) > toFloat(this.list().width());
      }
    ...
    list: function () {
      if (this._list === null) {
        ...
        this._list = $.isFunction(option)
          ? option.call(this)
          : this._element.find(option);
      }
    ...
    items: function () {
      if (this._items === null) {
        ...
        this._items = (
          $.isFunction(option) ? option.call(this) : this.list().find(option)
    ...
    dimension: function (element) {
      return toFloat(
        element["outer" + (this.vertical ? "Height" : "Width")](true)
      );
    },
        ...
        if (parsed.target > 0) {
          ...
          if (last >= end && this.tail) {
            ...
            if (
              (this.underflow &&
                current === end &&
                (wrap === "circular" || wrap === "both" || wrap === "last")) ||
              (!this.underflow &&
                last === end &&
                (wrap === "both" || wrap === "last"))
            ) {
              ...
              if (this.circular && index > end) {
                ...
                while (i++ < index) {
                  ...
                  if (isVisible) {
                    curr.after(
                      curr.clone(true).attr("data-jcarousel-clone", true)
                    );
                  }
          ...
          if (this.inTail) {
            this._scroll(
              Math.max(this.index(this._first) - scroll + 1, 0),
              animate,
              callback
            );
          } else {
    ...
    moveBy: function (properties, opts) {
      ...
      if (properties.left) {
        properties.left =
          toFloat(position.left) +
          correction +
          toFloat(properties.left) * multiplier +
          "px";
      }

      if (properties.top) {
        properties.top =
          toFloat(position.top) +
          correction +
          toFloat(properties.top) * multiplier +
          "px";
      }
    ...
    _position: function (item) {
      var first = this._first,
        ...
        center = this.options("center"),
        centerOffset = center
          ? this.clipping() / 2 - this.dimension(first) / 2
          : 0;
    ...
    _update: function (update) {
      var self = this,
        ...
        back =
          this.index(update.first || current.first) < this.index(current.first),
        key,
...
(function ($) {
  ...
  $.jcarousel.fn.scrollIntoView = function (target, animate, callback) {
    ...
    if (parsed.relative) {
      index =
        parsed.target < 0
          ? Math.max(0, first + parsed.target)
          : last + parsed.target;
    } else {
      index =
        typeof parsed.target !== "object"
          ? parsed.target
          : this.index(parsed.target);
    }
  ...
  $.jCarousel.plugin("jcarouselPagination", {
    ...
    _reload: function () {
      ...
      $.each(this._pages, function (page, carouselItems) {
        var currItem = (self._items[page] = $(
          item.call(self, page, carouselItems)
        ));

        currItem.on(
          self.options("event") + ".jcarouselpagination",
          $.proxy(function () {
            ...
            if (carousel.circular) {
              ...
              if (parseFloat(page) > parseFloat(self._currentPage)) {
                ...
                if (newIndex > currentIndex) {
                  target =
                    "-=" + (currentIndex + (numCarouselItems - newIndex));
                }
              }
            }
